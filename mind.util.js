...
Util.setCookie = function(b, c, a) {
  ...
  document.cookie =
    ...
    '=' +
    escape(c) +
    (a == null ? '' : ';path=/;expires=' + d.toGMTString())
}
  ...
  data: [
    {
      title: '思维导图全新升级',
      text:
        "<div style='margin:0px -20px;padding:50px 20px 80px;display:flex;justify-content:space-between;text-align:center;font-size:13px;background:#f3f3f3;'><div><img style='display:block;margin:20px auto;width:100px;' src='/assets/images/mind/guide1.svg'/>更简洁的UI<br>一切让您更专注创作</div><div><img style='display:block;margin:20px auto;width:100px;' src='/assets/images/mind/guide2.svg'/>增加更多实用功能<br>和快捷键支持</div><div><img style='display:block;margin:20px auto;width:100px;' src='/assets/images/mind/guide3.svg'/>多人实时协作<br>零延迟，高效易用</div><div><img style='display:block;margin:20px auto;width:100px;' src='/assets/images/mind/guide4.svg'/>更多下载格式的支持</div></div>"
    },
    {
      title: '一分钟学会思维导图 — 快速添加节点',
      img: '/assets/images/mind/step1.gif',
      text:
        "选中当前节点，使用 <span class='tutorial-key'>Enter</span>键增加同级节点，<span class='tutorial-key'>Tab</span>键增加子节点，<span class='tutorial-key'>Del</span>键增删除节点。"
    },
    {
      title: '移动节点位置和排序',
    ...
    },
    {
      ...
      text:
        "复用样式的时候，可以尝试使用<b>格式刷</b>功能，点击左上角的格式刷图标或者按快捷键 <span class='tutorial-key'>Ctrl</span> + <span class='tutorial-key'>g</span>"
    },
    {
      title: '善于用右键菜单来提高效率',
    ...
    },
    {
      ...
      text:
        '学会这些技巧，基本上可以轻松的玩转思维导图了<br>但想要更高效的使用思维导图，<b> 快捷键</b> 将是您需要了解的必备功能。您可以查看顶部菜单栏的 <b>帮助</b>，查看更多快捷键和帮助信息。'
    }
  ],
  dom: null,
  init: function(b) {
...
;(function() {
  ...
  $.fn.dropdown = function(options) {
    ...
    $(document)
      .off('mousedown.mind-dropdown')
      .on('mousedown.mind-dropdown', function() {
        menu.dropdown('close')
      })
    $(document)
      .off('mouseup.mind-dropdown')
      .on('mouseup.mind-dropdown', function() {
        menu.dropdown('close')
        if (options.onClose) {
          options.onClose()
        }
            ...
            $container
              ...
              .append(
                self.buildMonth(
                  new Date(
                    currentDate.getFullYear(),
                    4 * ($cell.parent().index() - 1) + $cell.index(),
                    1
                  )
                )
              )
        ...
        format: function(date) {
          if (options.dateFormat != null) {
            ...
            if (/(y+)/.test(options.dateFormat)) {
              result = options.dateFormat.replace(
                RegExp.$1,
  ...
  $.fn.confirm = function(options) {
    ...
    if (!confirmWin.length) {
      confirmWin = $(
        "<div id='mind-confirm-con'><div class='msg'></div><div class='buttons'><a class='okbtn'>确认</a>&nbsp;&nbsp;<a class='cancelbtn'>取消</a></div></div>"
      ).appendTo('body')
    }
  ...
  $.showTitleTip = function(title, delay, target_, pos) {
    ...
    if (tip.length == 0) {
      tip = $(
        "<div id='title_hover_tip'><div class='tip_arrow'></div><div class='tip_content'></div></div>"
      ).appendTo('body')
    }
  ...
  $.loading = function(options) {
    ...
    var loading = $(
      '<div id="mind-loading"><span><label style="color:#666;font-family: "comic sans ms","Microsoft YaHei";font-size: 22px;">ProcessOn</label> <label style="color:rgb(80, 194, 139);font-family: "comic sans ms","Microsoft YaHei";font-size: 22px;">mind</label><label style="display:inline-block;" class="mind-icons rotate1">&#xe64a;</label></span></div>'
    )
    $('body').append(loading)
  }
  $.ajaxSetup({
  ...
  $('[title],[original-title]')
    .live('mouseenter', function() {
      ...
      if (tip.length == 0) {
        tip = $(
          "<div id='mind_hover_tip'><div class='tip_arrow'></div><div class='tip_content'></div></div>"
        ).appendTo('body')
      }
      tip.find('.tip_content').html(title)
      $('#mind_hover_tip').show()
  ...
  $.fn.multiInput = function(opts, value, text) {
    ...
    input
      ...
      .on('keyup', function(e) {
        ...
        if (e.keyCode == 13 && opts.setVal) {
          var isEmail = /^([\w]+)(.[\w]+)*@([\w-]+\.){1,5}([A-Za-z]){2,4}$/.test(
            txt
          )
          if (!isEmail) {
            $(this).val('')
          ...
          if (opts.autoComplete) {
            ...
            $.ajax({
              ...
              success: function(data) {
                ...
                var htmlCon = $("<div class='popWindow'></div>").appendTo(
                  'body'
                )
                htmlCon.html(html)
                htmlCon.popWindow({
    ...
    function setVal(val, text) {
      ...
      var icon = '&#xe639;',
        isEmail = /^([\w]+)(.[\w]+)*@([\w-]+\.){1,5}([A-Za-z]){2,4}$/.test(text)
      if (isEmail && val == null) {
        icon = '&#xe609;'
        val = text
      } else {
      ...
      var html =
        '<span val="' +
        val +
        '" class="multi-input-value"><span class="mind-icons">' +
        icon +
        '</span><span class="multi-text">' +
        text +
        '</span><span class="mind-icons closeme">&#xe622;</span></span>'
      vals.append(html)
    }
  }
            ...
            $.get(
              ...
              function(data) {
                ...
                if (items.length == 0) {
                  ...
                  if (
                    opt.autoPosition &&
                    top + opt.offsetY + suggest.outerHeight() >
...
var popEditor = {
  ...
  renderMenu: function() {
    ...
    if (b.length == 0) {
      ...
      var c =
        "<div class='pop-editor'><div data-key='down'><span class='mind-icons icon-font'></span>" +
        d +
        "</div><div data-key='bold'><span class='mind-icons icon-bold'></span></div><div data-key='italic'><span class='mind-icons icon-italic'></span></div><div data-key='down'><span class='mind-icons icon-header'></span>" +
        a +
        "</div><div data-key='underline'><span class='mind-icons icon-underline'></span></div><div data-key='strikeThrough'><span style='font-size:17px;' class='mind-icons icon-zitizhonghuaxian'></span></div><div data-key='color'><span class='mind-icons icon-magic'></span></div><div data-key='insertUnorderedList'><span class='mind-icons icon-listul'></span></div><div data-key='insertOrderedList'><span class='mind-icons icon-listol'></span></div></div>"
      b = $(c).appendTo('body')
    }
  },
  renderMenuPos: function(c) {
    var b = c.parent(),
      a = $('.pop-editor')
    a.css({
